...
def get_info_on_loc(locations: List[Dict], parsed: List[Dict]) -> Union[str, List]:
    ...
    if len(results) == 1:
        ...
        best_matches = []
        for parse in parsed:
            best_match = get_close_matches(parse, possible_matches[i], 1)
            if best_match:
        ...
        places_found = [(index, result) for index, result in enumerate(best_matches)]
...
def multiple_choices(choices: List) -> Dict:
    ...
    print(
        f"Tu as tapé {answer}, tu vois bien que ça fait pas partie"
